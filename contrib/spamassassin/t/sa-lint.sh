...
#
# Loads Mailstrix.pm + strixd.pre (loadplugin) + strixd.cf (rules/meta) into the
...
exec docker run --rm -v "$root:/src" -w /src debian:trixie-slim sh -c '
  ...
  install -m644 contrib/spamassassin/strixd.pre "$d/strixd.pre"
  install -m644 contrib/spamassassin/strixd.cf  "$d/strixd.cf"
  spamassassin --lint '"$*"'
'
